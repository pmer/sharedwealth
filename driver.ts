...
import FakeNet from './fakeNet.js';
...
// Clients
const alice = new Client({name: 'Alice', net: fakeNet});
const bob = new Client({name: 'Bob', net: fakeNet});
const charlie = new Client({name: 'Charlie', net: fakeNet});

// Miners
const minnie = new Miner({name: 'Minnie', net: fakeNet});
const mickey = new Miner({name: 'Mickey', net: fakeNet});

// Creating genesis block
const genesis = Block.makeGenesis(new Map([
  [alice, 133],
  [bob, 99],
  [charlie, 67],
  [minnie, 400],
...
const donald = new Miner({name: 'Donald', net: fakeNet, startingBlock: genesis, miningRounds: 3000});

function showBalances(client: Client) {
